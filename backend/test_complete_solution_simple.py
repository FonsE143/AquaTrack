...
def test_complete_solution():
    print("Testing complete solution with StaffViewSet and DriverViewSet...")
    
    # Clean up any existing test users
    try:
        User.objects.filter(username__in=['test_staff_api', 'test_driver_api', 'testadmin_simple']).delete()
    ...
    try:
        admin_user = User.objects.get(username='testadmin_simple')
        print(f"Using existing admin user: {admin_user.username}")
    except User.DoesNotExist:
        # Create a test admin user
        try:
            admin_user = User.objects.create_user(
                username='testadmin_simple',
    ...
    request = factory.post('/api/staff/', staff_data, format='json')
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            try:
                profile = Profile.objects.get(user__username='test_staff_api')
    ...
    driver_data = {
        'username': 'test_driver_api',
        'first_name': 'Driver',
        'last_name': 'API',
        'email': 'driver.api@test.com',
        'phone': '09987654321'
    }
    ...
    request = factory.post('/api/drivers/', driver_data, format='json')
    request.user = admin_user
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            # Verify the profile was created with the correct role
            try:
                profile = Profile.objects.get(user__username='test_driver_api')
                print(f"Verified profile role: {profile.role}")
            except Profile.DoesNotExist:
                print("ERROR: Could not find created profile")
        else:
