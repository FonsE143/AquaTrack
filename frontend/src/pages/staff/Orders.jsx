...
export default function StaffOrders(){
  ...
  const orders = (data?.results || data || []).filter(order => 
  ...
  const handleStatusChange = (orderId, newStatus) => {
    // For "Out for Delivery", we also need to ensure driver assignment is handled properly
    const order = orders.find(o => o.id === orderId);
    if (newStatus === 'out') {
      // If changing to "Out for Delivery", include the current driver if one is assigned
      const payload = { status: newStatus };
      if (order && order.driver) {
        payload.driver_id = order.driver.id;
      }
  ...
  const handleDriverChange = (orderId, driverId, currentStatus) => {
    console.log('Assigning driver to order:', { orderId, driverId, currentStatus });
    const payload = { 
                        ...
                        <tr key={o.id}>
                          {o.isEmpty ? (
                            ...
                            <>
                              ...
                              <td>{`${o.customer_first_name || ''} ${o.customer_last_name || ''}`.trim() || o.customer_username || o.customer}</td>
